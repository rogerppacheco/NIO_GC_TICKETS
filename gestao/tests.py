...
class ComissionamentoTests(TestCase):
    ...
    def test_formatar_html_email_comissionamento(self):
        from gestao.models import LoteImportacao, RelatorioComissionamento
        from gestao.messaging.envio import formatar_html_email_comissionamento
        from gestao.pipelines.comissionamento import processar_comissionamento
        ...
        html_out = formatar_html_email_comissionamento(rel, "Samuel Pires")
        self.assertIn("Olá, <strong>Samuel Pires</strong>!", html_out)
        self.assertIn("<strong>Comissionamento por PDV</strong>", html_out)
        self.assertIn("PDV: <strong>INOVA MG</strong>", html_out)
        self.assertIn("LUISA SERVICOS DE TELEFONIA MOVEL LTDA", html_out)
        self.assertIn("Orientação para envio do email", html_out)
        self.assertIn("#fefce8", html_out)  # Bloco de destaque para instruções

    def test_enviar_lote_email_view(self):
        from unittest.mock import patch
        from django.contrib.auth import get_user_model
        from gestao.models import LoteImportacao, RelatorioComissionamento
        from gestao.pipelines.comissionamento import processar_comissionamento
        ...
        User = get_user_model()
        user = User.objects.create_superuser("admin_lote", "admin_lote@teste.com", "senha123")
        PerfilStaff.objects.create(user=user, papel=PerfilStaff.Papel.GESTOR)
        esp = User.objects.create(username="samuel_lote", email="samuel@niointernet.com.br")
        self.pdv.especialista = esp
        self.pdv.save(update_fields=["especialista"])
        self.client.force_login(user)
        ...
        with patch("gestao.messaging.envio.smtp_configurado", return_value=True), \
            ...
            resp = self.client.post(
                "/gestao/comissionamento/?escopo=todos",
                {"action": "enviar_lote_email", "lote": lote.id, "escopo": "todos"},
                follow=True,
            )
            self.assertEqual(resp.status_code, 200)
...
class ResultadosTests(TestCase):
    ...
    def test_top_e_piores_sem_repetir(self):
        ...
        # Total: A 6, B 6, C 4, VISION 2 — empate A/B resolve por % (A 300% > B 200%)
        linhas = [
        ...
        # Top por total: A, B, C, E, VISION
        self.assertEqual([l["pdv"] for l in top6], ["A", "B", "C", "E", "VISION"])
        # Bottom do restante após top: só F
    ...
    def test_top_desempate_por_pct_plano(self):
        from gestao.pipelines.parcial_vendas import _top_e_piores

        # Mesmo total: maior % do plano fica na frente
        linhas = [
            {"parceiro_id": 1, "pdv": "PEQ", "vendas": 10, "plano": 10, "pct_plano": 1.0, "elegivel": True},
            {"parceiro_id": 2, "pdv": "GRD", "vendas": 10, "plano": 5, "pct_plano": 2.0, "elegivel": True},
        ]
        top, _ = _top_e_piores(linhas)
        self.assertEqual(top[0]["pdv"], "GRD")
        self.assertEqual(top[1]["pdv"], "PEQ")

        # Bottom: mesmo total baixo — menor % entra primeiro
        bottom_linhas = [
            {"parceiro_id": 1, "pdv": "TOPA", "vendas": 20, "plano": 5, "pct_plano": 4.0, "elegivel": True},
            {"parceiro_id": 2, "pdv": "TOPB", "vendas": 18, "plano": 5, "pct_plano": 3.6, "elegivel": True},
            {"parceiro_id": 3, "pdv": "TOPC", "vendas": 16, "plano": 5, "pct_plano": 3.2, "elegivel": True},
            {"parceiro_id": 4, "pdv": "TOPD", "vendas": 14, "plano": 5, "pct_plano": 2.8, "elegivel": True},
            {"parceiro_id": 5, "pdv": "TOPE", "vendas": 12, "plano": 5, "pct_plano": 2.4, "elegivel": True},
            {"parceiro_id": 6, "pdv": "BAIXO_ALTO_PCT", "vendas": 1, "plano": 2, "pct_plano": 0.5, "elegivel": True},
            {"parceiro_id": 7, "pdv": "BAIXO_BAIXO_PCT", "vendas": 1, "plano": 10, "pct_plano": 0.1, "elegivel": True},
        ]
        _, pior = _top_e_piores(bottom_linhas)
        self.assertEqual([l["pdv"] for l in pior], ["BAIXO_BAIXO_PCT", "BAIXO_ALTO_PCT"])
